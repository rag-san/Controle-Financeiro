...
import { parseDashboardTrendsQuery } from "@/app/api/dashboard/_query";

export async function GET(request: NextRequest): Promise<NextResponse> {
  return withRouteProfiling(request, "/api/dashboard/patrimony.GET", async () => {
    const auth = await requireUser(request);
    if (auth instanceof NextResponse) return auth;

    const rawQuery = Object.fromEntries(request.nextUrl.searchParams.entries());
    const cacheQuery = request.nextUrl.searchParams.toString();
    const cacheKey = `dashboard:${auth.userId}:patrimony:${cacheQuery || "default"}`;
    ...
    let parsed;
    try {
      parsed = parseDashboardTrendsQuery(rawQuery);
    } catch (error) {
    ...
    const payload = await dashboardMetricsRepo.getPatrimony({
      userId: auth.userId,
      range: parsed.range,
