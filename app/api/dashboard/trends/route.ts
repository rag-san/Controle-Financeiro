...
export async function GET(request: NextRequest): Promise<NextResponse> {
  return withRouteProfiling(request, "/api/dashboard/trends.GET", async () => {
    const auth = await requireUser(request);
    if (auth instanceof NextResponse) return auth;

    const rawQuery = Object.fromEntries(request.nextUrl.searchParams.entries());
    const cacheQuery = request.nextUrl.searchParams.toString();
    const cacheKey = `dashboard:${auth.userId}:trends:${cacheQuery || "default"}`;
    ...
    const payload = await dashboardMetricsRepo.getTrends({
      userId: auth.userId,
      range: parsed.range,
