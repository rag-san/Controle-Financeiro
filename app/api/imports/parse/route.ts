...
import { PdfImportError, parsePdfImport } from "@/lib/pdf";
...
function unsupportedSourceResponse(
      ...
      if (sourceType === "pdf") {
        ...
        try {
          ...
          if (error instanceof PdfImportError) {
            ...
            const fallbackMessage =
              error.code === "no_transactions_found"
                ? error.message
                : "Nao foi possivel ler este PDF automaticamente. Tente CSV/OFX ou um PDF com layout de extrato mais simples.";
            ...
            return unsupportedSourceResponse(sourceType, fallbackMessage, error.technicalReason, fallbackCode);
          }
          ...
          logImportEvent("import.parse", {
            ...
            errorRows: 1,
            phase: "parse",
            errorCode: "source_parser_unavailable"
          });

          return unsupportedSourceResponse(sourceType, message, technicalReason);
        }
      }
