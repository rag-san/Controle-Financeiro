...
export function NotificationItem({
  ...
  return (
    <li
      className={[
        "rounded-xl border border-border bg-card/90 p-3 shadow-sm transition hover:border-border hover:shadow-md",
        "dark:border-border dark:bg-card/90 dark:hover:border-border",
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="min-w-0 flex-1">
          <div className="flex items-start justify-between gap-2">
            <p className="text-sm font-semibold leading-5 text-foreground">{insight.title}</p>
            <span className={`shrink-0 rounded-full px-2 py-0.5 text-[10px] font-semibold ${badge.className}`}>
              {badge.label}
            </span>
          </div>

          <p className="mt-1 text-xs leading-5 text-muted-foreground [display:-webkit-box] [-webkit-box-orient:vertical] [-webkit-line-clamp:3] overflow-hidden">
            {insight.message}
          </p>
